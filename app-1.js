...
ko.applyBindings(myViewModel);
  ...
  function initMap() {
    ...
    var myAddress={lat: 40.7413549, lng: -73.9980244};
  ...
  function populateInfoWindow(marker, infowindow) {
    ...
    if (infowindow.marker != marker) {
      ...
      function getNYInfo(){
        ...
        var url = "https://api.nytimes.com/svc/search/v2/articlesearch.json?q="+loc
         +'&sort=newest&api-key=4eb556c803c1437a9cf825e50eaac64f';

         $.getJSON(url,function(data){

           var article=data.response.docs[0];
           //console.log(article);
           nyNews= '<div><a href='+article.web_url+'>'
           +article.headline.main+'</a>'+'<p>'+article.snippet+'</p></div>';//return the first news about the place
         }).error(function(){
        nyNews= "<div>the NEW YORK TIMES API service is not good...</div>";
      });
      }
